"""
migrate_crossref.py — Observatorio CCHEN 360°
===============================================
Migra cchen_crossref_enriched.csv → tabla crossref_enrichment en Supabase.
PK: doi
"""
import os, sys, math
...
CSV_PATH = ROOT / "Data" / "Publications" / "cchen_crossref_enriched.csv"
...
def migrate():
    ...
    COLS = ["doi","crossref_funders","crossref_funder_doi","references_count",
            "cited_by_crossref","abstract","license_url","publisher","subject"]
    df = df[[c for c in COLS if c in df.columns]]
    df = df.dropna(subset=["doi"]).drop_duplicates(subset=["doi"])

    for c in ("references_count","cited_by_crossref"):
        if c in df.columns:
    ...
    for i in range(batches):
        ...
        try:
            sb.table("crossref_enrichment").upsert(chunk, on_conflict="doi").execute()
