"""
migrate_inspire.py — Observatorio CCHEN 360°
=============================================
Migra cchen_inspire_works.csv a la tabla inspire_works en Supabase.
...
USO:
    cd /home/bastin/Dropbox/CCHEN
    PYTHONUNBUFFERED=1 python3 -u Database/migrate_inspire.py
"""
...
CSV_PATH = ROOT / "Data" / "Publications" / "cchen_inspire_works.csv"
...
def migrate_inspire():
    ...
    cols = [
        "inspire_id", "arxiv_id", "doi", "title", "authors",
        "journal", "year", "pub_date", "abstract", "keywords",
        "citation_count", "document_type", "inspire_categories",
        "affiliation_raw", "inspire_url", "fetched_at",
    ]
    df = df[[c for c in cols if c in df.columns]]
    df = df.dropna(subset=["inspire_id"])
    df = df.drop_duplicates(subset=["inspire_id"])
    ...
    for i in range(batches):
        ...
        try:
            supabase.table("inspire_works").upsert(
                chunk, on_conflict="inspire_id"
            ).execute()
    ...
    return {"table": "inspire_works", "rows": total, "batches": batches, "errors": errors}


if __name__ == "__main__":
    result = migrate_inspire()
