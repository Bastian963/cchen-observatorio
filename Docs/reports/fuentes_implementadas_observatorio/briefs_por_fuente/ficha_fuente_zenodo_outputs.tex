...
\hypersetup{colorlinks=true, linkcolor=cchenpurple, urlcolor=cchenpurple,
  pdftitle={Ficha de fuente de información - Zenodo},
  pdfauthor={CCHEN 360}}
\urlstyle{same}
\pagestyle{fancy}
\fancyhf{}
...
\begin{center}
{\fontsize{16}{17.5}\selectfont\bfseries\color{cchenpurple}Zenodo}\\[-1pt]
{\fontsize{9.1}{10.4}\selectfont\color{textgray}Ficha de fuente de información para datos relacionados con CCHEN}\\[-1pt]
{\fontsize{7.8}{9}\selectfont\color{mutedgray}Código: \texttt{zenodo} \quad | \quad Categoría: Científica}
\end{center}
\vspace{0.01cm}
...
Repositorio generalista de CERN/OpenAIRE para conjuntos de datos, presentaciones, programas, afiches y otros resultados de investigación.

\sectiontitle{2. Qué datos se descargaron}
\begin{calloutbox}{Tipología de datos}
Metadatos de conjuntos de datos, presentaciones y archivos asociados
\end{calloutbox}
\textbf{Datos descargados para CCHEN.} CSV de metadatos Zenodo y CSV de inventario de archivos; no descarga archivos binarios.

\sectiontitle{3. Cómo se filtra}
Aliases institucionales CCHEN visibles en afiliacion o metadatos; solo metadatos, sin descarga de archivos.

\sectiontitle{4. Dónde quedó guardado}
\begin{itemize}
\item \path{Data/ResearchOutputs/cchen_zenodo_metadata.csv}
\item \path{Data/ResearchOutputs/cchen_zenodo_files.csv}
\item \path{Data/ResearchOutputs/zenodo_cchen_state.json}
\item \path{Data/Gobernanza/curaduria_zenodo_cchen.csv}
\end{itemize}

\sectiontitle{5. Estadística de descarga}
Registros obtenidos/evidencia: 68. Archivos locales generados o registrados: 4. Calidad registrada: 1.0. Última actualización: 2026-05-19. Estado de corrida: success.

\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}\raggedright
\sectiontitle{6. Para qué sirve en el observatorio}
\begin{itemize}
  \item \textbf{Uso principal:} Inventariar metadatos de conjuntos de datos, presentaciones y resultados CCHEN en Zenodo sin descargar archivos.
  \item \textbf{Potencial:} Inventariar metadatos de conjuntos de datos, presentaciones y resultados CCHEN en Zenodo sin descargar archivos.
\end{itemize}
...
\vspace{0.02cm}
{\scriptsize\color{mutedgray}Sitio: https://zenodo.org \quad | \quad Interfaz de datos: https://developers.zenodo.org/}
