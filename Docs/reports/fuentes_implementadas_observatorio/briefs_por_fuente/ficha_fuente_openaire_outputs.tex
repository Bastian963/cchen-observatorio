...
\hypersetup{colorlinks=true, linkcolor=cchenpurple, urlcolor=cchenpurple,
  pdftitle={Ficha de fuente de información - OpenAIRE},
  pdfauthor={CCHEN 360}}
\urlstyle{same}
\pagestyle{fancy}
\fancyhf{}
...
\begin{center}
{\fontsize{16}{17.5}\selectfont\bfseries\color{cchenpurple}OpenAIRE}\\[-1pt]
{\fontsize{9.1}{10.4}\selectfont\color{textgray}Ficha de fuente de información para datos relacionados con CCHEN}\\[-1pt]
{\fontsize{7.8}{9}\selectfont\color{mutedgray}Código: \texttt{openaire} \quad | \quad Categoría: Científica}
\end{center}
\vspace{0.01cm}
...
Infraestructura europea de ciencia abierta. Integra publicaciones, conjuntos de datos, proyectos, repositorios y relaciones entre resultados.

\sectiontitle{2. Qué datos se descargaron}
\begin{calloutbox}{Tipología de datos}
Resultados académicos, repositorios y relaciones ORCID/DOI
\end{calloutbox}
\textbf{Datos descargados para CCHEN.} CSV de resultados académicos y repositorios asociados a identificadores CCHEN.
...
\begin{itemize}
\item \path{Data/ResearchOutputs/cchen_openaire_outputs.csv}
\item \path{Data/ResearchOutputs/openaire_state.json}
\end{itemize}

\sectiontitle{5. Estadística de descarga}
Registros obtenidos/evidencia: 878. Archivos locales generados o registrados: 2. Calidad registrada: 1.0. Última actualización: 2026-05-11. Estado de corrida: seeded\_from\_resultados.

\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}\raggedright
\sectiontitle{6. Para qué sirve en el observatorio}
\begin{itemize}
  \item \textbf{Uso principal:} Consolidar resultados academicos europeos y repositorios vinculados a ORCID/DOI CCHEN.
  \item \textbf{Potencial:} Consolidar resultados academicos europeos y repositorios vinculados a ORCID/DOI CCHEN.
\end{itemize}
...
\vspace{0.02cm}
{\scriptsize\color{mutedgray}Sitio: https://www.openaire.eu/ \quad | \quad Interfaz de datos: https://api.openaire.eu/graph/v2/researchProducts}
