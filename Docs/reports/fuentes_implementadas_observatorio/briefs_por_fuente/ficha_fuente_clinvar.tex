...
\hypersetup{colorlinks=true, linkcolor=cchenpurple, urlcolor=cchenpurple,
  pdftitle={Ficha de fuente de información - ClinVar},
  pdfauthor={CCHEN 360}}
\urlstyle{same}
\pagestyle{fancy}
\fancyhf{}
...
\begin{center}
{\fontsize{16}{17.5}\selectfont\bfseries\color{cchenpurple}ClinVar}\\[-1pt]
{\fontsize{9.1}{10.4}\selectfont\color{textgray}Ficha de fuente de información para datos relacionados con CCHEN}\\[-1pt]
{\fontsize{7.8}{9}\selectfont\color{mutedgray}Código: \texttt{clinvar} \quad | \quad Categoría: Bio/Farma}
\end{center}
\vspace{0.01cm}
...
{\Large\bfseries varios archivos: 20; 875}\par
...
{\large\bfseries semestral (otros resultados: trimestral)}\par
...
{\normalsize\bfseries Implementada derivada/semilla}\par
...
{\normalsize\bfseries Mantener con nota}\par
...
Base biomédica de variantes clínicas. En esta etapa aparece solo como evidencia derivada de flujos de vida y salud.

\sectiontitle{2. Qué datos se descargaron}
\begin{calloutbox}{Tipología de datos}
Variantes clínicas relacionadas por flujos biomédicos derivados
\end{calloutbox}
\textbf{Datos descargados para CCHEN.} No hay extractor directo ClinVar; la evidencia viene de PubMed/radiofarmacia como aproximacion temática.
...
\begin{itemize}
\item \path{Data/Publications/cchen_pubmed_works.csv}
\item \path{Data/Publications/pubmed_state.json}
\item \path{Data/Gobernanza/radiofarmacia_cchen_seeds.csv}
\item \path{Data/Gobernanza/radiofarmacia_cchen_pubchem_compounds.csv}
\end{itemize}

\sectiontitle{5. Estadística de descarga}
Registros obtenidos/evidencia: varios archivos: 20; 875. Archivos locales generados o registrados: 14. Calidad registrada: 1.0. Última actualización: 2026-05-11; 2026-05-19. Estado de corrida: seeded\_from\_resultados; success. Los valores múltiples corresponden a artefactos distintos; no deben sumarse como una sola tabla.

\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}\raggedright
\sectiontitle{6. Para qué sirve en el observatorio}
\begin{itemize}
  \item \textbf{Uso principal:} Sirve como evidencia temática o vigilancia exploratoria; no como indicador directo de la fuente original.
  \item \textbf{Potencial:} Vigilancia tecnica en radiofarmacia, moleculas, radionuclidos y literatura biomédica relacionada con lineas CCHEN.
\end{itemize}

\sectiontitle{7. Estado operativo}
\begin{calloutbox}{Estado}
Implementada como derivada/semilla; requiere nota metodologica al usarla.
\end{calloutbox}
Estado catalogo: implementada en registro operativo. Ultima corrida: seeded\_from\_resultados; success; ultima actualizacion: 2026-05-11; 2026-05-19.

\sectiontitle{8. Debilidades y riesgos}
Aparece como implementada por registro operativo, pero su origen en la matriz no era API priorizada; mantener trazabilidad de metodo y resultados.

\sectiontitle{9. Decisión}
Mantener como evidencia derivada o semilla. No presentarla como extractor directo hasta implementar conexion propia.
...
\vspace{0.02cm}
{\scriptsize\color{mutedgray}Sitio: https://www.ncbi.nlm.nih.gov/clinvar/ \quad | \quad Interfaz de datos: sin interfaz registrada}
