...
\hypersetup{colorlinks=true, linkcolor=cchenpurple, urlcolor=cchenpurple,
  pdftitle={Ficha de fuente de información - Europe PMC},
  pdfauthor={CCHEN 360}}
\urlstyle{same}
\pagestyle{fancy}
\fancyhf{}
...
\begin{center}
{\fontsize{16}{17.5}\selectfont\bfseries\color{cchenpurple}Europe PMC}\\[-1pt]
{\fontsize{9.1}{10.4}\selectfont\color{textgray}Ficha de fuente de información para datos relacionados con CCHEN}\\[-1pt]
{\fontsize{7.8}{9}\selectfont\color{mutedgray}Código: \texttt{europe\_pmc} \quad | \quad Categoría: Científica}
\end{center}
\vspace{0.01cm}
...
Repositorio y buscador biomédico europeo. Complementa PubMed con literatura, identificadores y enlaces a texto completo.

\sectiontitle{2. Qué datos se descargaron}
\begin{calloutbox}{Tipología de datos}
Publicaciones biomédicas y metadatos Europe PMC
\end{calloutbox}
\textbf{Datos descargados para CCHEN.} CSV de publicaciones biomédicas filtradas por CCHEN, autores, afiliaciones o DOI conocidos.
...
\begin{itemize}
\item \path{Data/Publications/cchen_europmc_works.csv}
\item \path{Data/Publications/europmc_state.json}
\end{itemize}

\sectiontitle{5. Estadística de descarga}
Registros obtenidos/evidencia: 74. Archivos locales generados o registrados: 2. Calidad registrada: 1.0. Última actualización: 2026-03-22. Estado de corrida: seeded\_from\_resultados.

\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}\raggedright
\sectiontitle{6. Para qué sirve en el observatorio}
\begin{itemize}
  \item \textbf{Uso principal:} Complementar publicaciones biomédicas y de radiofarmacia con metadatos Europe PMC.
  \item \textbf{Potencial:} Complementar publicaciones biomédicas y de radiofarmacia con metadatos Europe PMC.
\end{itemize}
...
\vspace{0.02cm}
{\scriptsize\color{mutedgray}Sitio: https://europepmc.org/ \quad | \quad Interfaz de datos: https://europepmc.org/RestfulWebService}
