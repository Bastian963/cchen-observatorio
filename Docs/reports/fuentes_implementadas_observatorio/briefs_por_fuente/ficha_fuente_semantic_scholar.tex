...
\hypersetup{colorlinks=true, linkcolor=cchenpurple, urlcolor=cchenpurple,
  pdftitle={Ficha de fuente de información - Semantic Scholar},
  pdfauthor={CCHEN 360}}
\urlstyle{same}
\pagestyle{fancy}
\fancyhf{}
...
\begin{center}
{\fontsize{16}{17.5}\selectfont\bfseries\color{cchenpurple}Semantic Scholar}\\[-1pt]
{\fontsize{9.1}{10.4}\selectfont\color{textgray}Ficha de fuente de información para datos relacionados con CCHEN}\\[-1pt]
{\fontsize{7.8}{9}\selectfont\color{mutedgray}Código: \texttt{semantic\_scholar} \quad | \quad Categoría: Científica}
\end{center}
\vspace{0.01cm}
...
Grafo académico con metadatos enriquecidos, autores, citas y relaciones semánticas entre publicaciones.

\sectiontitle{2. Qué datos se descargaron}
\begin{calloutbox}{Tipología de datos}
Metadatos académicos, autores, citas y relaciones semanticas
\end{calloutbox}
\textbf{Datos descargados para CCHEN.} CSV de metadatos/citas Semantic Scholar asociados a publicaciones CCHEN.
...
\begin{itemize}
\item \path{Data/Publications/cchen_semantic_scholar.csv}
\end{itemize}

\sectiontitle{5. Estadística de descarga}
Registros obtenidos/evidencia: 1057. Archivos locales generados o registrados: 1. Calidad registrada: 1.0. Última actualización: 2026-05-11. Estado de corrida: seeded\_from\_resultados.

\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}\raggedright
\sectiontitle{6. Para qué sirve en el observatorio}
\begin{itemize}
  \item \textbf{Uso principal:} Complementar citas, autores y metadatos semanticos de publicaciones CCHEN.
  \item \textbf{Potencial:} Complementar citas, autores y metadatos semanticos de publicaciones CCHEN.
\end{itemize}
...
\vspace{0.02cm}
{\scriptsize\color{mutedgray}Sitio: https://semanticscholar.org \quad | \quad Interfaz de datos: https://www.semanticscholar.org/product/api}
