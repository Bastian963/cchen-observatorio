...
\hypersetup{colorlinks=true, linkcolor=cchenpurple, urlcolor=cchenpurple,
  pdftitle={Ficha de fuente de información - Unpaywall},
  pdfauthor={CCHEN 360}}
\urlstyle{same}
\pagestyle{fancy}
\fancyhf{}
...
\begin{center}
{\fontsize{16}{17.5}\selectfont\bfseries\color{cchenpurple}Unpaywall}\\[-1pt]
{\fontsize{9.1}{10.4}\selectfont\color{textgray}Ficha de fuente de información para datos relacionados con CCHEN}\\[-1pt]
{\fontsize{7.8}{9}\selectfont\color{mutedgray}Código: \texttt{unpaywall} \quad | \quad Categoría: Científica}
\end{center}
\vspace{0.01cm}
...
Servicio abierto que informa si un DOI tiene versión de acceso abierto y dónde puede encontrarse.

\sectiontitle{2. Qué datos se descargaron}
\begin{calloutbox}{Tipología de datos}
Estado de acceso abierto por DOI
\end{calloutbox}
\textbf{Datos descargados para CCHEN.} CSV con estado open access por DOI CCHEN.
...
\begin{itemize}
\item \path{Data/Publications/cchen_unpaywall_oa.csv}
\end{itemize}

\sectiontitle{5. Estadística de descarga}
Registros obtenidos/evidencia: 780. Archivos locales generados o registrados: 1. Calidad registrada: 1.0. Última actualización: 2026-05-19. Estado de corrida: success.

\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}\raggedright
\sectiontitle{6. Para qué sirve en el observatorio}
\begin{itemize}
  \item \textbf{Uso principal:} Determinar disponibilidad open access de DOI CCHEN.
  \item \textbf{Potencial:} Determinar disponibilidad open access de DOI CCHEN.
\end{itemize}
...
\vspace{0.02cm}
{\scriptsize\color{mutedgray}Sitio: https://unpaywall.org \quad | \quad Interfaz de datos: https://unpaywall.org/products/api}
