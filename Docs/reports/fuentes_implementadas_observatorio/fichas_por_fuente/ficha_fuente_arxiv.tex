...
\hypersetup{colorlinks=true, linkcolor=cchenpurple, urlcolor=cchenpurple,
  pdftitle={Ficha de fuente de información - arXiv},
  pdfauthor={CCHEN 360}}
\urlstyle{same}
\pagestyle{fancy}
\fancyhf{}
...
\begin{center}
{\fontsize{16}{17.5}\selectfont\bfseries\color{cchenpurple}arXiv}\\[-1pt]
{\fontsize{9.1}{10.4}\selectfont\color{textgray}Ficha de fuente de información para datos relacionados con CCHEN}\\[-1pt]
{\fontsize{7.8}{9}\selectfont\color{mutedgray}Código: \texttt{arxiv} \quad | \quad Categoría: Científica}
\end{center}
\vspace{0.01cm}
...
{\Large\bfseries varios archivos: 407; 1}\par
...
{\large\bfseries semanal (otros resultados: semestral)}\par
...
Repositorio abierto de preprints académicos. Sirve para detectar producción temprana antes de su publicación formal.

\sectiontitle{2. Qué datos se descargaron}
\begin{calloutbox}{Tipología de datos}
Preprints, metadatos bibliograficos y vigilancia temprana
\end{calloutbox}
\textbf{Datos descargados para CCHEN.} CSV de vigilancia semanal y CSV de trabajos arXiv CCHEN cuando hay DOI/autor/alias asociado.
...
\begin{itemize}
\item \path{Data/Vigilancia/arxiv_monitor.csv}
\item \path{Data/Publications/cchen_arxiv_works.csv}
\item \path{Data/Publications/arxiv_state.json}
\end{itemize}

\sectiontitle{5. Estadística de descarga}
Registros obtenidos/evidencia: varios archivos: 407; 1. Archivos locales generados o registrados: 3. Calidad registrada: 1.0. Última actualización: 2026-05-19; 2026-05-11. Estado de corrida: success; seeded\_from\_resultados. Los valores múltiples corresponden a artefactos distintos; no deben sumarse como una sola tabla.

\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}\raggedright
\sectiontitle{6. Para qué sirve en el observatorio}
\begin{itemize}
  \item \textbf{Uso principal:} Vigilar preprints y produccion temprana en física y areas afines CCHEN.
  \item \textbf{Potencial:} Vigilar preprints y produccion temprana en física y areas afines CCHEN.
\end{itemize}
...
\end{calloutbox}
Estado catalogo: implementada en registro operativo. Ultima corrida: success; seeded\_from\_resultados; ultima actualizacion: 2026-05-19; 2026-05-11.
...
\vspace{0.02cm}
{\scriptsize\color{mutedgray}Sitio: https://arxiv.org \quad | \quad Interfaz de datos: https://info.arxiv.org/help/api/index.html}
